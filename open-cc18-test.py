...
if __name__ == "__main__":
  ...
  if not os.path.isdir("open-cc18-gama-ft"):
    os.mkdir("open-cc18-gama-ft")
  ...
  for _, t_id in zip(trange(len(benchmark_suite.tasks)), benchmark_suite.tasks):
    ...
    for fold_nr, (train_idx, test_idx) in enumerate(cv):
      ...
      clf = GamaClassifier(scoring=scorer_name, max_total_time=180, max_eval_time=120, n_jobs=-1, post_processing=EnsemblePostProcessing())
      clf.fit(X_train, y_train)
      y_proba = clf.predict_proba(X_test)
      score = scorer(y_test, y_proba[:,1])

      with open(f"open-cc18-gama-ft/{dataset.id}_{fold_nr}.json", "w") as f:
        fold_res = {
          "data_id": dataset.id,
          "name": dataset.name,
          "metric": scorer_name,
